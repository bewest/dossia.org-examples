...
if(isset($_REQUEST['record_id']) && !empty($_REQUEST['record_id'])){
	...
	switch($info['content_type']){
		case 'image/gif':			
			$fp = fopen('output/api_output.gif', 'w');
			fwrite_stream($fp,$api_output_data);
			fclose($fp);
			$link = '<img src="output/api_output.gif"/>';
			break;
		case 'image/jpeg':
			$fp = fopen('output/api_output.jpg', 'w');
			fwrite_stream($fp,$api_output_data);
			fclose($fp);
			$link = '<img src="output/api_output.gif"/>';
			break;		
		case 'image/bmp':
			$fp = fopen('output/api_output.bmp', 'w');
			fwrite_stream($fp,$api_output_data);
			fclose($fp);
			$link = '<img src="output/api_output.gif"/>';
			break;			
		case 'application/pdf':
			$fp = fopen('output/api_output.pdf', 'w');
			fwrite_stream($fp,$api_output_data);
			fclose($fp);
			$link = '< a href="output/api_output.pdf"> Click to view the output </a>';
			break;
	}	
	curl_close($ch);
}
            ...
            <table width="100%" border="0" cellspacing="6" cellpadding="0">
				...
				<tr>
					   <td width="30%" align="right">
						...
						<td width="70%">
								<input type="text" name="doc_id" value="<?=$_REQUEST['doc_id']?>" size="50"/>
						  </td>
				</tr>
				<tr>
					   <td width="30%" align="right">
                ...
                <tr>
                    <td align="right" valign="top">
                            API Resource Output Data:
                    </td>
                    <td>
                        <textarea cols="70" rows="12"><?php echo $api_output_data;?></textarea>
                    </td>
                </tr>
                <tr>
                    <td align="right" valign="top">
